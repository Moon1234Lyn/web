(function (factory) {
	...
	// var domain = '';
	// if(Url.indexOf('/api/dp/webapi')!=-1){
	// 	domain = Url.replace('/api/dp/webapi','');
	// }else{
	// 	domain = Url;
	// }
    /*************************策略对象*****************************/
    ...
    var RULES = {
        ...
        equalTo: function (value, value1, errorMsg) {
            if (document.getElementById(value1).value != value) {
                return errorMsg;
            }
        },
		checkEntIsExists: function (value, value1, errorMsg) {
			var companyName= document.getElementById(value1).value;
            if(value != "" && value1 != ""){
				var b = true;
				var message = '';
				$.ajax({
					type:"get",
					url:domain+"/api/dp/check/checkEntIsExists",
					async:false,
					data:{
		...
		checkPhoneExists: function (value, errorMsg) {
			var b = true;
			var message = '';
			$.ajax({
				type:"get",
				url:domain+"/api/dp/user/isMobileAndEmailExists",
				async:false,
				data:{
					mobile: value
				},
		...
		checkEmailExists: function (value, errorMsg) {
			var b = true;
			var message = '';
			$.ajax({
				type:"get",
